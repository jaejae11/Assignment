...
int staffRegister()
{
	...
	int askForMenu = ZERO;
	int nameValidation = ZERO, positionValidation = ZERO, genderValidation = ZERO;
	...
	do
	{
		...
		do
		{
			positionValidation = ZERO;
			printf("Position                :");
			rewind(stdin);
			if (scanf(" %15[a-zA-Z]", &registerAccount.position) == ONE)
			{
				positionValidation += ONE;
			}
			else
			{
				printf("Error: Invalid input!\n\n");
			}
		} while (positionValidation != ONE);
		...
		while (fscanf(registerACC, "%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^\n]\n", &finding.require.ID, &finding.require.passWord, &finding.name, &finding.gender, &finding.contactNumber,& finding.position) != EOF)
		{
		...
		{
			printf("Account Register Successful\n");
			fprintf(registerACC2, "%s:%s:%s:%c:%s:%s\n", registerAccount.require.ID, registerAccount.require.passWord, registerAccount.name, registerAccount.gender, registerAccount.contactNumber, registerAccount.position);
			fclose(registerACC2);
...
int staffModify(StaffInformationDetail modifyFunction)
{
	...
	do
	{
		...
		if (choice == ONE)
		{
			...
			scanf("%[^\n]", &modifyData.name);
			rewind(stdin);
			while (fscanf(modifyACC, "%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^\n]\n", &finding[i].require.ID, &finding[i].require.passWord, &finding[i].name, &finding[i].gender, &finding[i].contactNumber, &finding[i].position) != EOF)
			{
		...
		{
			fprintf(writeIntoFIle, "%s:%s:%s:%c:%s:%s\n", finding[j].require.ID, finding[j].require.passWord, finding[j].name, finding[j].gender, finding[j].contactNumber, finding[j].position);
		}
...
int staffSearch(StaffInformationDetail searchFunction)
{
	...
	do
	{
		...
		displayInfo = fopen("staff.txt", "r");
			...
			while (fscanf(displayInfo, "%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^\n]\n", &finding.require.ID, &finding.require.passWord, &finding.name, &finding.gender, &finding.contactNumber, &finding.position) != EOF)
			{
				if (strcmp(finding.require.ID, idInput) == ZERO)
				{
					printf(" ID                     : %s\n", finding.require.ID);
					printf(" Name                   : %s\n", finding.name);
					printf(" Gender                 : %c\n", finding.gender);
...
void displayBooking()
{
	...
	while (fscanf(displayuse, "%[^:]:%[^:]:%[^:]:%[^:]:%[^:]:%[^\n]\n", &display.facilityId, &display.facilityName, &display.userId, &display.bookingdate, &display.bookingtime, &display.bookingtimeuntil) != EOF)
	{
		...
		printf(" User ID       :%s\n", display.userId);
		printf(" Booking Date  :%s\n", display.bookingdate);
		printf(" Booking Time  : %s-%s\n", display.bookingtime, display.bookingtimeuntil);
		printf("---------------------------------------------------------\n");
	}
	puts("");
...
void searchFacility()
{
	...
	Facility facilityMenu[30] = { 0 }, check;
	FILE* display;
	display = fopen("facilitymenu.txt", "r");
	if (!display)
	{
	...
	do {
		...
		if (choice == 1)
		{
			...
			rewind(stdin);
			scanf("%[^\n]", &check.facilityType);
			for (i = 0, found = 0; i < 30 && found != 1; i++)
			{
				if (strcmp(check.facilityType, facilityMenu[i].facilityType) == 0)
				{
					printf("Facility ID         Facility Type            Venue        Facility Allowed Number User\n");
					printf("==========================================================================================\n");
					printf("|||||||              |||||||||||             |||||           |||||||||||||||||||\n");
				...
				else if (strcmp(check.facilityType, facilityMenu[i].facilityType) != 0 && i == 29)
				{
		...
		else if (choice == 2)
		{
			...
			for (i = 0, found = 0; i < 30 && found != 1; i++)
			{
				if (strcmp(check.facilityID, facilityMenu[i].facilityID) == 0)
				{
					printf("Facility ID         Facility Type            Venue        Facility Allowed Number User\n");
					printf("==========================================================================================\n");
					printf("|||||||              |||||||||||             |||||           |||||||||||||||||||\n");
				...
				else if (strcmp(check.facilityID, facilityMenu[i].facilityID) != 0 && i == 29)
				{
					printf("____________________________________________\n");
					printf("No result\n");
					printf("The facility is not found in the Facility Menu !\n");
...
void addFacility()
{
	...
	do {
		...
		if (yesno == 'Y' || yesno == 'y')
		{
			...
			printf("Desc : %s\n", check.facilityDesc);
			do {
				printf("Are you confirm to add this new facility into the Facility Menu(Y/N)\n");
				rewind(stdin);
				scanf("%c", &yesno2);
		...
		if (yesno == 'Y' || yesno == 'y')
		{
			...
			do {
				...
				if (choice == 1)
				{
					...
					for (i = 0, found = 0; i < 30 && found != 1; i++)
					{
						if (strcmp(searchFacilityType, facilityMenu[i].facilityType) == 0)
						{
							modifyFacilitySub(i, facilityMenu);
							found = 1;
						}
						else if (strcmp(searchFacilityType, facilityMenu[i].facilityType) != 0 && i == 29)
						{
							printf("____________________________________________\n");
							printf("No result\n");
							printf("The facility is not found in the Facility Menu !\n");
				...
				else if (choice == 2)
				{
					...
					for (i = 0, found = 0; i < 30 && found != 1; i++)
					{
						if (strcmp(searchFacilityID, facilityMenu[i].facilityID) == 0)
						{
							modifyFacilitySub(i, facilityMenu);
							found = 1;
						}
						else if (strcmp(searchFacilityID, facilityMenu[i].facilityType) != 0 && i == 29)
						{
							printf("____________________________________________\n");
							printf("No result\n");
							printf("The facility is not found in the Facility Menu !\n");
...
void modifyFacilitySub(int i, Facility facilityMenu[30])
{
	...
	for (i = 0; i < 30 && facilityMenu[i].totalUsers != 0; i++)
	{
...
void addFacilityUsage()
{
	...
	{
		if (strcmp(addFunction.finding.idInput, addFunction.userId) == 0)
		{
